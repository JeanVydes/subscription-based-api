...
use crate::{server::AppState, account::{Account, GenericResponse}, subscription::{Subscription, SubscriptionFrequencyClass, SubscriptionHistoryLog}, helpers::random_string};
...
pub async fn subscription_created(event: SubscriptionEvent, state: Arc<AppState>) -> Result<(), Json<GenericResponse>> {
    ...
    let mut history_logs = customer.suscription.history_logs.clone();
    history_logs.push(SubscriptionHistoryLog{
        id: subscription_id.clone(),
        subscription_plan_id: event.data.attributes.product_id.clone(),
        frequency,
        starts_at: event.data.attributes.created_at.clone(),
        ends_at: event.data.attributes.ends_at.clone(),
        renews_at: event.data.attributes.renews_at.clone(),
    });
    ...
    let update_subscription = Subscription{
        ...
        frequency,
        active: event.data.attributes.status == "active",
